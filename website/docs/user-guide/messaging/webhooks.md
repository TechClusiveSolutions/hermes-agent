...
| `approval_delegate` | No | Forward this route's ambiguous-command approval prompts (the ones `approvals.mode: smart` would otherwise leave waiting for a human) to a real interactive platform instead of always failing closed. Does not affect genuinely dangerous commands, which the Smart DENY classifier still denies outright regardless of this setting — but note it does make the ambiguous bucket human-approvable rather than guaranteed-denied. Button-only: the target platform's adapter must support interactive approvals (Slack, Discord, Telegram, etc.), and "Always Allow" is never offered on a delegated prompt. Format: `"<platform>:<chat-or-channel-id>"`, e.g. `"slack:C0B8JK868SX"`. Overrides the global `approvals.delegate` default for this route — and on multiplexed `/p/<profile>/` routes it's the **only** way to enable delegation, since profile-stamped routes never inherit the global default (that would leak one profile's approvals config onto another's route). See [Approval Delegation for Headless Sessions](../security.md#approval-delegation-for-headless-sessions). |
| `approval_delegate_timeout_seconds` | No | Per-route override of how long a delegated approval waits for a reply before falling back to the default fail-closed timeout. Overrides the global `approvals.headless_timeout_seconds` (which, like the delegate target, is not inherited on `/p/<profile>/` routes). Only meaningful alongside `approval_delegate`. |
| `approval_delegate_metadata` | No | Optional dict forwarded verbatim to the delegate adapter's `send_exec_approval` — needed when the adapter routes sends via metadata, e.g. multi-workspace Slack installs select the workspace-scoped client from `team_id`: `approval_delegate_metadata: {team_id: T0123ABC}`. Without it on such installs, the prompt is sent on the primary workspace's token and delegation fails closed. Overrides the global `approvals.delegate_metadata`. Single-workspace installs don't need it. |
